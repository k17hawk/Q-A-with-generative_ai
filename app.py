...
get_api_key = st.sidebar.text_input("Enter your OpenAI API key:", type="password")

# Dropdown to select the models
llm = st.sidebar.selectbox("Select an OpenAI model", ["gpt-4", "gpt-4-turbo", "gpt-4o"])
...
if user_input:
    if get_api_key:
        response = generate_response(user_input, get_api_key, llm, temperature, max_tokens)
        st.write(response)
    else:
